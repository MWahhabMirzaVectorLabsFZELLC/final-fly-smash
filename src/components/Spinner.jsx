...
const Spinner = ({ setScore, onClose }) => {
  ...
  useEffect(() => {
    ...
    // Load the remaining spins from local storage or set to 3
  ...
  const onFinished = (winner) => {
    ...
    // Decrease spins count and check if out of spins
    setRemainingSpins((prev) => {
      ...
      if (newSpins <= 0) {
        ...
        setIsDisabled(true); // Disable spinning
      }
      return newSpins;
    });
    ...
    >
      ...
      >
        <WheelComponent
          ...
          downDuration={400}
          fontFamily="bold"
          disabled={spinning || isDisabled || remainingSpins <= 0} // Disable if no spins left
        />
      </div>
    </div>
  );
